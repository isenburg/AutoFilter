...
## 4. DX-Filter, Sortierbare Pipeline & VIP-Whitelists

Die rechte Filter-Sidebar bietet eine frei sortierbare First-Match Filter-Pipeline mit 12 Sektionen. Die Auswertung erfolgt streng **von oben nach unten**:

### Das First-Match Boolean-Prinzip:
1. **VIP-Whitelists (Treffer → Sofort-Pass)**:
   Trifft ein Signal auf einen VIP-Filter (*Erlaubte Maidenhead-Grids*, *Erlaubte DX-Rufzeichen*, *Erlaubte DX-Länder*), erhält es sofort einen **VIP-Pass (`return true`)** und überspringt alle nachfolgenden Sperren.
2. **VIP-Whitelists (Nicht-Treffer → Neutraler Durchlauf)**:
   Trifft ein Signal **nicht** auf die VIP-Whitelist zu, wird es **nicht blockiert**, sondern neutral zur nächsten Filter-Sektion nach unten weitergeleitet.
3. **Blacklists (Treffer → Sofort-Block)**:
   Trifft ein Signal auf eine aktivierte Sperre (*Gesperrte Länder*, *Gesperrte Kontinente*, *Zonen*, *Worked Before*, *Grid*, *Doubletten*, *Most Wanted*), wird es sofort blockiert (**`return false`**).
4. **Ende der Pipeline**:
   Stationen, die keine Sperre auslösen, passieren am Ende normal (**`return true`**).

### Die Filter-Sektionen im Überblick (Standard-Reihenfolge):
1. **VIP: Erlaubte Maidenhead-Grids**: Whitelist-Modus für Einzel-Grids (`DN71`), Ranges (`DN61-DN74`, `KN64-KN71`) und Listen (`DN61-DN74, EN10`).
2. **VIP: Erlaubte DX-Rufzeichen**: Whitelist-Modus nach Rufzeichen-Präfixen (z. B. `DP0`, `K1`).
3. **VIP: Erlaubte DX-Länder**: Whitelist-Modus mit sofortigem VIP-Pass bei Treffer und neutraler Weiterleitung bei Nicht-Treffer.
4. **Kontinent-Filter**: Schaltet einzelne Kontinente (AF, AN, AS, EU, NA, OC, SA) flexibel an oder aus.
5. **Gesperrte Länder (Blacklist)**: Blockiert Signale aus ausgewählten DXCC-Ländern.
6. **Gesperrte CQ-Zonen**: Blockiert gezielt Stationen aus bestimmten CQ-Zonen (1–40).
7. **Gesperrte ITU-Zonen**: Blockiert gezielt Stationen aus bestimmten ITU-Zonen (1–90).
8. **Most Wanted Only**: Lässt ausschließlich unbestätigte Most-Wanted-Entitäten bis zum gewählten Rang zu.
9. **Gearbeitete Stationen (Worked Before)**: Lässt Stationen nach Ablauf einer frei wählbaren Zeitspanne (0–999 Stunden, Tage, Monate, Jahre) wieder zu.
10. **Maidenhead Grid-Filter**: Filtert nach neuen, noch nicht gearbeiteten 4-Stellen- (`JO31`) oder 6-Stellen-Grids (`JO31aa`).
11. **WSJT-X CQ Filter**: Lässt nur Anrufe durch, die `CQ`, `RR73`, `RRR` oder `73` enthalten.
12. **Doubletten-Filter**: Verhindert doppelte Dekodierungen des gleichen Rufzeichens auf derselben Frequenz innerhalb eines konfigurierbaren Zeitfensters (1–15 Min.) und Frequenztoleranz (0.5–3.0 kHz).
